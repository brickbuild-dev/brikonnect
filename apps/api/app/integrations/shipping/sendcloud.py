...
class SendCloudAdapter:
    ...
    async def create_label(self, shipment: ShipmentRequest) -> LabelResponse:
        shipment_id = f"sc_{uuid.uuid4().hex}"
        tracking = f"SC{uuid.uuid4().hex[:10].upper()}"
        label_url = f"https://labels.sendcloud.fake/{shipment_id}.pdf"
