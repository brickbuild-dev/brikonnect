...
async def test_tenant_isolation(db_session):
    ...
    user_a = await create_user(
        db_session,
        tenant_a.id,
        UserCreate(email="owner@alpha.local", password="alpha123"),
        hash_password("alpha123"),
    )
    user_b = await create_user(
        db_session,
        tenant_b.id,
        UserCreate(email="owner@beta.local", password="beta123"),
    ...
    async with AsyncClient(app=app, base_url="http://test", headers=headers) as ac:
        login = await ac.post("/api/v1/auth/login", json={"email": "owner@alpha.local", "password": "alpha123"})
        assert login.status_code == 200

        users = await ac.get("/api/v1/users")
        assert users.status_code == 200
        emails = {u["email"] for u in users.json()}
        assert "owner@alpha.local" in emails
        assert "owner@beta.local" not in emails
